...
<?php foreach ($days as $day_key => $day_title): ?>
  ...
  <table class="session-calendar">
    <tr>
      ...
      <?php $node = node_load($room_nid); ?>
        <th><span class="room-label"><?php print $room['title']; ?></span><?php if(!empty($room['sponsor'])): ?><div class="sponsor-label"><?php print $room['sponsor']; ?></div><?php endif; ?><br /><?php if(!empty($room['capacity'])): ?><div class="capacity-label"><span>Capacity:</span> <?php print $node->field_room_capacity[0]['value']; ?></div><?php endif; ?></th>
      <?php endif; ?>
      <?php endforeach ?>
    </tr>
